...
import aula_02.ListaDuplamenteEncadeada;

/* Utilizando a lista duplamente encadeada criada em aula com o código compartilhado, 
 * otimize a função de busca com o nome get de forma que se a posição for do início ao meio da lista, 
 * verifique a partir do início, já caso a posição seja maior que o meio, 
 * verifique a partir do final, caso ache o item, retorne sem finalizar a iteração.*/

public class Exercicio_01<T> {
	...
	public T validateRulesList(int posicao) throws Exception {
		if (posicao < 0)
			throw new Exception("Posição não pode ser negativa");
		if (posicao >= tamanho)
			throw new IndexOutOfBoundsException("Posição acima do tamanho da lista");
